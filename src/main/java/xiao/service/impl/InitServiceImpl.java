...
import xiao.dao.BaseDaoI;
import xiao.model.Tmodule;
import xiao.model.TmoduleType;
...
*/
	...
	private void initResource(Document document) {
		...
		for (Object obj : childNodes) {
			Node node = (Node) obj;
			Tmodule module = (Tmodule) baseDao.getById(Tmodule.class, node.valueOf("@id"));
			if (module == null) {
				module = new Tmodule();
			}
			module.setId(node.valueOf("@id"));
			module.setName(node.valueOf("@name"));
			module.setUrl(node.valueOf("@url"));
			module.setDescription(node.valueOf("@description"));
			module.setIconCls(node.valueOf("@iconCls"));
			module.setSeq(Integer.parseInt(node.valueOf("@seq")));

			if (!StringUtils.isBlank(node.valueOf("@pid"))) {
				module.setModule((Tmodule) baseDao.getById(Tmodule.class, node.valueOf("@pid")));
			} else {
				module.setModule(null);
			}

			Node n = node.selectSingleNode("resourcetype");
			TmoduleType type = (TmoduleType) baseDao.getById(TmoduleType.class, n.valueOf("@id"));
			if (type != null) {
				module.setModuleType(type);
			}

			logger.info(JSON.toJSONStringWithDateFormat(module, "yyyy-MM-dd HH:mm:ss"));
